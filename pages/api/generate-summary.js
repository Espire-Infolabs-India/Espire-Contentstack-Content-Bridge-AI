...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      let rootGlobalFields =
        schemas?.filter((field) => field?.data_type == "global_field") || [];
      ...
      let textSchemaObjectsForChatBot =
        allSchemaObjects?.filter(
          (field) =>
            field?.data_type == "text" || field?.data_type == "rich_text"
        ) || [];
      let fileSchemaObjectsForChatBot =
        allSchemaObjects?.filter((field) => field?.data_type == "file") || [];
      let referenceObjects =
        allSchemaObjects?.filter((field) => field?.data_type == "reference") ||
        [];
      ...
      if (referenceObjects && referenceObjects != undefined) {
        await Promise.all(
          referenceObjects?.map(async (field) => {
      ...
      if (selectedModel == "custom_bot") {
        ...
        axios(config).then(function (response) {
          ...
          if (Array.isArray(parsedTemp)) {
            ...
            let finalFieldsArray = [
              ...mergedResponse,
              ...fileSchemaObjectsForChatBot,
              ...refrerenceFieldsList,
            ];
            const groupedOutput = groupFieldsByParentUid(finalFieldsArray);
            res
              .status(200)
